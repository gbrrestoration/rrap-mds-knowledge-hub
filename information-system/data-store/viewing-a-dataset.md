...
## Viewing a dataset
Users can view a dataset a number of ways once they are logged into the RRAP M&DS IS Data Store.
...
Users are then taken to the Datasets page, where they can:
* Quickly filter datasets by organisation by selecting the check boxes beside each name
* Select the dataset they are interested in by clicking the dataset title 
* Refresh their search by clicking the **Refresh list** button
...
Once users have found the dataset they are looking for, they click on the dataset name and will then be taken to the dataset's metadata page.
